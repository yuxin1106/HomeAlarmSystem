...
/* Address of the VCNL light sensor on the I2C bus */
#define VCNL_ADDR 0x26 //26 for write, 27 for read


uint8_t rdata[2];              // Buffer for reading data from vcnl
uint8_t adata[1];
uint8_t wdata[1];
uint8_t w_adata[1];
//#define OSTR_SIZE   80o   

volatile char binary[16];
...
void bcd_to_binary(uint8_t a, int start)
{
    uint8_t i;
    int cursor = 0;
    ...
    for (i=0x80; i!=0; i>>=1)
    {
       //lcd_moveto(0,cursor+start);
       if (a&i)
       {
          //lcd_writedata('1');
          binary[cursor+start] = 1;
       }
       else
       {
          //lcd_writedata('0');
          binary[cursor+start] = 0;
       }
...
unsigned short int binary_to_decimal()
{
    int i = 0;
    ...
    for (i=0; i<16; i++)
    {
       decimal = decimal + binary[i]*pow(2, 15-i);   
    }
    return decimal;
}
...
//     //read 16 bits result from register at 87h & 88h
//     lcd_moveto(0,0);
//     lcd_stringout("test");
...
//         char buff[17];
//         lcd_moveto(1,0);
//         snprintf(buff, 17, "%6d", decimal);
//         lcd_stringout(buff);
//         lcd_moveto(1,5);
//         if(decimal>5000){
//             lcd_stringout("bigger");
//         }
//         else{
//             lcd_stringout("smaller");
//         }
...
//         lcd_writecommand(1);
//         start_vcnl();  
//         unsigned short int result = read_vcnl();
//         lcd_moveto(0,0);
//         if(result>5000){
//             lcd_stringout("window closed");
//         }
//         else{
//             lcd_stringout("window open");
//         }
//     }
...
void start_vcnl(){
    wdata[0] = 0x0A;
    w_adata[0] = 0x80;   // command register 80h
    i2c_io(VCNL_ADDR, w_adata, 1, wdata, 1, NULL, 0); 
}

unsigned short int read_vcnl(){
    memset(rdata, 0, 16);
    adata[0] = 0x87;
    i2c_io(VCNL_ADDR, adata, 1, NULL, 0, rdata, 2); 

    bcd_to_binary(rdata[0],0);
    bcd_to_binary(rdata[1],8);
    unsigned short int decimal = binary_to_decimal();
    return decimal;
}
